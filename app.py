...
@app.route("/")
def index():
    return jsonify({"app": "ACEest Fitness & Gym",
                    "version": "3.2.4", "status": "running",
                    "programs": list(PROGRAMS.keys())}), 200
...
def get_program(program_name):
    matched = next((k for k in PROGRAMS
                    if k.lower().replace(" ", "-") ==
                    program_name.lower().replace(" ", "-")), None)
    if not matched:
        return jsonify({"error": "Program not found"}), 404
    return jsonify({"program": matched,
                    "details": PROGRAMS[matched]}), 200
...
def add_client():
    ...
    conn.close()
    return jsonify({"message": f"Client '{name}' saved",
...
def get_progress(name):
    ...
    return jsonify({"client": name,
                    "progress": [dict(r) for r in rows]}), 200
...
def estimate_calories():
    ...
    return jsonify({"weight": weight, "program": program,
                    "calories": calculate_calories(weight, program)}), 200
